...
def send_queued_messages(reddit, database):
	count_pending_messages = database.get_count_pending_messages()

	messages_sent = 0
	if count_pending_messages > 0:
		if count_pending_messages == 0:
			messages_to_send = 0
		elif count_pending_messages < 200:
			messages_to_send = 30
		else:
			messages_to_send = min(1000, int(count_pending_messages / 5))

		messages = database.get_pending_messages(messages_to_send)
		for message in messages:
			messages_sent += 1
			log.info(
				f"{messages_sent}/{len(messages)}/{count_pending_messages}: Notifying u/"
				f"{message.subscription.subscriber.name} for u/{message.subscription.author.name} in r/"
				f"{message.subscription.subreddit.name} : {message.submission.submission_id}")

			bldr = utils.get_footer(message.render_message())
			result = reddit.send_message(message.subscription.subscriber.name, "UpdateMeBot Here!", ''.join(bldr))
			if result in (ReturnType.INVALID_USER, ReturnType.USER_DOESNT_EXIST):
				log.info(f"User doesn't exist: u/{message.subscription.subscriber.name}")

			if message.subscription.recurring:
